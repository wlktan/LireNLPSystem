...
#' @param text.cols The text columns in segmented.reports, defaults to c("body", "impression")
...
CreateTextFeatures <- function(segmented.reports,
  ...
  ### create feature matrix for each text column
    for(col in 1:length(text.cols)){
      ...
      this.dfm <- corpus(as.character(segmented.reports[,text.cols[col]]),
        ...
        dfm_trim(.,
                 min_count = min_count,
                 min_docfreq = min_doc,
                 max_docfreq = dim(segmented.reports)[1]/2) %>%
        as.data.frame(.)
